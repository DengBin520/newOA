		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<?php if(is_array($newData)): foreach($newData as $key=>$val): if(is_array($val)): foreach($val as $key=>$v): ?><tr class="text-c">
							...
							<td><?php echo ($v["oa_tool_name"]); ?></td>
							<td><?php echo ($v["oa_tool_number"]); ?></td>
							<td><?php echo ($v["oa_the_number"]); ?></td>
							<td><?php echo (date("Y-m-d",$v["oa_buy_datatime"])); ?></td>
							<td><?php echo ($v["oa_tool_price"]); ?></td>
							...
							<td><?php echo ($v["oa_tool_state"]); ?></td>
							<td class="text-c"><?php echo ($v["oa_user_name"]); ?></td>
							<td><?php echo (date("Y-m-d",$v["oa_tool_add_time"])); ?></td>
							...
							<a title="编辑" href="javascript:;" onclick="member_edit('编辑','','<?php echo ($v["tid"]); ?>','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a style="text-decoration:none" class="ml-5" onClick="picture_del(this,'<?php echo ($v["tid"]); ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
$('.table-sort').dataTable({
	...
	"aoColumnDefs": [
	  ...
	  {"orderable":false,"aTargets":[1,2,6,7,8,10,11]}// 制定列不参与排序
	]
});
...
function member_edit(title,url,id,w,h){
	url = '/ader_oa/admin/Tool/tool_update?tid='+id;
...
function picture_del(obj,id){console.log(id);
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Tool/tool_delete',
			data:{"tid":id},
			dataType: 'json',
