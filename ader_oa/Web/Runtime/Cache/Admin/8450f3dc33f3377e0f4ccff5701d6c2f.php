...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <!-- <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> --><a href="javascript:;" onclick="admin_add('添加管理员','admin-add.html','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 分配权限组</a></span>  <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($newData)): foreach($newData as $key=>$val): if(is_array($val)): foreach($val as $key=>$v): ?><tr class="text-c">
					...
					<a title="取消该管理员" href="javascript:;" onclick="admin_upd(this,'<?php echo ($v["uid"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe631;</i></a></td>
...
function admin_add(title,url,w,h){
	var url = '/ader_oa/admin/Atube/admin_group_access';
...
function admin_upd(obj,id){
	layer.confirm('确认要取消吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Atube/admin_upd',
			data: {"id":id},
			dataType: 'json',
			success: function(data){
				$(obj).parents("tr").remove();
				layer.msg('已取消管理资格!',{icon:1,time:1000});
			},
			error:function(data) {
