...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><!-- <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> --> <a href="javascript:;" onclick="admin_permission_add('添加权限节点','admin-permission-add.html','','400')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限组</a></span> <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr class="text-c">
				...
				<td class="td-manage">
				<?php if($v["status"] == '1'): ?><a style="text-decoration:none" onClick="admin_stop(this,'<?php echo ($v["id"]); ?>')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
				<?php else: ?>
				<a onClick="admin_start(this,<?php echo ($v["id"]); ?>)" href="javascript:;" title="启用" style="text-decoration:none"><i class="Hui-iconfont">&#xe615;</i></a><?php endif; ?>
				<a title="修改" href="javascript:;" onclick="admin_permission_edit('权限编辑','','<?php echo ($v["id"]); ?>','','310')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
				<a title="删除" href="javascript:;" onclick="admin_permission_del(this,'<?php echo ($v["id"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function admin_permission_add(title,url,w,h){
	var url = '/ader_oa/admin/Atube/admin_group_add';
	layer_show(title,url,w,h);
}
/*管理员-权限-修改*/
function admin_permission_edit(title,url,id,w,h){
	var url = '/ader_oa/admin/Atube/admin_permission_edit?id='+id;
	layer_show(title,url,w,h);
}
...
function admin_permission_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Atube/admin_permission_del',
			data: {"id":id},
			dataType: 'json',
...
/*管理员-停用*/
function admin_stop(obj,id){console.log(id);
	layer.confirm('确认要停用吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
		// var url="{:url('Atube/admin_stop')?id="+id;
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Atube/admin_stop',
			data:{"id":id},
			dataType: 'json',
			success:function(data){
...
/*管理员-启用*/
function admin_start(obj,id){console.log(id);
	layer.confirm('确认要启用吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Atube/admin_start',
			data:{"id":id},
			dataType: 'json',
			success:function(data){
