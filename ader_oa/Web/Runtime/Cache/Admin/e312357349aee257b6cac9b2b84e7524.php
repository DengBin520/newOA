...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($newAdd)): foreach($newAdd as $key=>$v): ?><tr class="text-c">
					<td><?php echo ($v["zid"]); ?></td>
					<td><?php echo (date("Y-m-d",$v["oa_date_time"])); ?></u></td>
					<td><?php echo ($v["oa_matters"]); ?></td>
					<td>-<?php echo ($v["oa_num_ber"]); ?>分</td>
					<td><?php echo ($v["oa_operation_name"]); ?></td>
					<td class="text-l"><?php echo (date("Y-m-d",$v["oa_add_time"])); ?></td>
					<td class="td-manage"> <!-- <a title="编辑" href="javascript:;" onclick="member_edit('编辑','member-add.html','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  --> <a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo ($v["zid"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Integral/rules_jian_delete',
			data:{"id":id},
			dataType: 'json',
			success: function(data){
