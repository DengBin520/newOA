...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="picture_add('添加图片','picture-add.html')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a></span> <span class="r">共有数据：<strong><?php echo ($newCount); ?></strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<?php if(is_array($newData)): foreach($newData as $key=>$val): if(is_array($val)): foreach($val as $key=>$v): ?><tr class="text-c">
							...
							<td><?php echo ($v["oa_project_name"]); ?></td>
							<td><?php echo ($v["oa_company_url"]); ?></td>
							<td><?php echo ($v["oa_customer_name"]); ?></td>
							<td><?php echo ($v["oa_customer_tel"]); ?></td>
							<td><?php echo ($v["oa_user_name"]); ?></td>
							<!-- <td><a href="javascript:;" onClick="picture_edit('图库编辑','','<?php echo ($v["tid"]); ?>')">
							<img width="210" class="picture-thumb" src="/ader_oa/Public/photo/images/<?php echo ($v["oa_tool_img"]); ?>"></a></td> -->
							<td><?php echo ($v["oa_tel"]); ?></td>
							<td><?php echo (date("Y-m-d",$v["oa_project_add_time"])); ?></td>
							<td><?php echo (date("Y-m-d",$v["oa_project_end_time"])); ?></td>

							<td class="text-c"><?php echo ($v["oa_location"]); ?></td>

							<td><?php echo (date("Y-m-d",$v["oa_add_time"])); ?></td>
							<td class="td-status"><?php echo ($v["oa_comment"]); ?><!-- <span class="label label-success radius">已发布</span> --></td>
							<td class="td-manage">
							...
							<a title="编辑" href="javascript:;" onclick="member_edit('编辑','','<?php echo ($v["pid"]); ?>','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a style="text-decoration:none" class="ml-5" onClick="picture_del(this,'<?php echo ($v["pid"]); ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
$('.table-sort').dataTable({
	...
	"aoColumnDefs": [
	  ...
	  {"orderable":false,"aTargets":[1,2,3,4,5,11,12]}// 制定列不参与排序
	]
});
...
function member_edit(title,url,id,w,h){
	url = '/ader_oa/admin/Project/project_update?pid='+id;
	layer_show(title,url,w,h);
}
/*图片-删除*/
function picture_del(obj,id){console.log(id);
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Project/project_delete',
			data:{"pid":id},
			dataType: 'json',
