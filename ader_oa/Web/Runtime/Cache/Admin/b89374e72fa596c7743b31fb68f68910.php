...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><!-- <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> --> <a href="javascript:;" onclick="member_add('修改积分','member-add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 修改积分</a></span> <span class="r">共有数据：<strong><?php echo ($newCount); ?></strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<?php if(is_array($newData)): foreach($newData as $key=>$val): if(is_array($val)): foreach($val as $key=>$v): ?><tr class="text-c">
					...
					<td><u style="cursor:pointer" class="text-primary" onclick="member_show('查看','','<?php echo ($v["uid"]); ?>','360','400')"><?php echo ($v["oa_user_name"]); ?></u></td>
					...
					<td class="td-manage"> <!-- <a title="编辑" href="javascript:;" onclick="member_edit('编辑','member-add.html','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  --> <a title="删除" href="javascript:;" onclick="member_del(this,'<?php echo ($v["gid"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
$(function(){
	$('.table-sort').dataTable({
		...
		"aoColumnDefs": [
		  ...
		  {"orderable":false,"aTargets":[0,2,4,6,8]}// 制定列不参与排序
		]
	});
...
function member_add(title,url,w,h){
	var url = '/ader_oa/admin/Integral/integral';
	layer_show(title,url,w,h);
}
/*用户-查看*/
function member_show(title,url,id,w,h){
	url = '/ader_oa/admin/Integral/integral_user_list?uid='+id;
	layer_show(title,url,w,h);
}
/*用户-停用*/
...
function member_del(obj,id){console.log(id);
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '/ader_oa/admin/Integral/integral_delete',
			data:{"id":id},
			dataType: 'json',
